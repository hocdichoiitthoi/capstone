...
__all__ = ['optimize_with_sage', 'SAGE_ATTN_AVAILABLE']
...
def sage_attention(
    ...
    Args:
        ...
        config: SageAttention configuration
    ...
    # Quantize Q and K to INT8 if enabled
    if config.use_int8:
    ...
    if config.dropout > 0 and not config.deterministic:
        attn_weights = F.dropout(attn_weights, p=config.dropout)
    
    # Compute output with block-sparse attention if sequence is long
    if seq_len_k > config.block_size:
        output = torch.zeros_like(q)
        for i in range(0, seq_len_k, config.block_size):
            end_idx = min(i + config.block_size, seq_len_k)
...
def optimize_with_sage(
    ...
    ) if sage_config is None else SageConfig.from_dict(sage_config)
